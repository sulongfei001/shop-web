...
class ActivityList4 extends Page {
    constructor(props) {
        super(props);
        this.state = {
            showTitle: false,
    ...
    btnMouseDown = () => {
        let { btn } = this.state;
        btn.style.backgroundColor = "#0285AD";
        this.setState({
            btn: btn
        });
    }
    btnMouseUp = () => {
        let { btn } = this.state;
        btn.style.backgroundColor = "#0299C7";
        this.setState({
            btn: btn
        });
    }
    ...
    render() {
        ...
        return (
            <div>
                {!showTitle &&
                    <TopTitle title="粉丝会预约报名" style={{ backgroundColor: "" }} onClickBack={() => { history.goBack(); }} />}
                <TransitionGroup>
                    {showTitle && <Fade>
                        <TopTitle title="粉丝会预约报名" style={{ backgroundColor: "#333333" }} onClickBack={() => { history.goBack(); }} />
                    </Fade>}
                </TransitionGroup>
                <FullScreenPage style={{ background: '#B8D5F2', zIndex: -1 }} />
                <div className="ActivityList4">
